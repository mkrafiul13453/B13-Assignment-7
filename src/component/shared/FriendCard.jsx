...
const FriendCard = ({ friend }) => {
    // স্ট্যাটাস অনুযায়ী আলাদা আলাদা কালার সেট করা
    const statusColors = {
        "overdue": "bg-red-500",
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center transition-all hover:shadow-md">
            ...
            {/* স্ট্যাটাস ব্যাজ */}
            <span className={`${statusColors[friend.status] || 'bg-gray-500'} text-white text-xs font-semibold px-4 py-1 rounded-full capitalize`}>
                {friend.status}
            </span>
        </div>
    );
